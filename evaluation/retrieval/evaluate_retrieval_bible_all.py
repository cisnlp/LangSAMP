...
# update below
import copy
from modeling_xlmr_decoupled import XLMRobertaDecoupledModel
...
def prepare_batch(sentences, tokenizer, model_type, device="cuda", max_length=512, lang='eng_Latn', langid=None, use_local_max_length=True, pool_skip_special_token=False):
    ...
    if model_type == "xlm":
        ...
        return {"input_ids": input_ids, "attention_mask": attention_mask}, pool_mask
...
# updated below
def tokenize_text(args, text_file, tok_file, tokenizer, lang=None):
...
def load_model(args, lang, output_hidden_states=None):
    ...
    if not args.init_checkpoint:
        ...
        if args.init_checkpoint == 'cis-lmu/glot500-base':
            ...
            model = XLMRobertaDecoupledModel.from_pretrained(args.init_checkpoint, config=config)
    print(config)
    print(model)
...
# updated below
def extract_embeddings(args, model, tokenizer, text_file, tok_file, embed_file, lang='eng_Latn', pool_type='mean', langid=0):
    ...
    sent_toks = tokenize_text(args, text_file, tok_file, tokenizer, lang)
...
# updated below
def extract_encodings(args, model, tokenizer, text_file, tok_file, embed_file, lang='eng_Latn',
                      max_seq_length=None, langid=0):
    ...
    with open(os.path.join(args.output_dir, f'test_results.txt'), 'w') as result_writer:
        for src_lang in args.predict_langs:
            ...
            src_text_file = os.path.join(
                data_dir, 'bible.{}-{}.{}'.format(src_lang, tgt_lang, src_lang))
            if not os.path.exists(src_text_file):
                continue
            tgt_text_file = os.path.join(
                data_dir, 'bible.{}-{}.{}'.format(src_lang, tgt_lang, tgt_lang))
            ...
            tgt_tok_file = os.path.join(
                args.tokenized_dir, '{}-eng.tok.eng'.format(src_lang))

            all_src_embeds = extract_embeddings(
                args, model, tokenizer, src_text_file, src_tok_file, None, lang=src_lang, pool_type=args.pool_type, langid=langid)
            all_tgt_embeds = extract_embeddings(
                args, model, tokenizer, tgt_text_file, tgt_tok_file, None, lang=tgt_lang, pool_type=args.pool_type, langid=langid)
