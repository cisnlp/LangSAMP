...
def tokenize_preprocess(args):
    ...
    if not args.init_checkpoint:
        ...
        print(f"Loading {args.init_checkpoint.split('/')[-1]} tokenizer ...")
        tokenizer = XLMRobertaTokenizer.from_pretrained(args.init_checkpoint, 
            do_lower_case=args.do_lower_case, cache_dir=args.cache_dir if args.cache_dir else None)
...
def main(): 
    ...
    all_models = [f for f in os.listdir(args.init_checkpoint) if f != 'runs']
    all_models.insert(0, "checkpoint-0")

    main_model_dir = args.init_checkpoint
    main_out_dir = args.output_dir
    main_tokenized_dir = args.tokenized_dir

    main_out_dir = main_out_dir + f"/{main_model_dir.split('/')[-1]}"

    for i, model_name in enumerate(all_models):
        print(f"Current model: {model_name}...")

        args.output_dir = main_out_dir + f"/{model_name}"

        if i == 0:
            assert model_name == "checkpoint-0"
            args.init_checkpoint = 'cis-lmu/glot500-base'
        else:
            args.init_checkpoint = main_model_dir + f"/{model_name}"

        args.tokenized_dir = main_tokenized_dir + f"/{model_name}"
        args.model_save_name = f"/{model_name}"
